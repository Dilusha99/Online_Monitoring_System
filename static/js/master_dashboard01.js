...
/**
 * Update plant buttons with live data including individual unit status
 */
function updatePlantButtons(data) {
    ...
    buttons.forEach(btn => {
        ...
        // Update individual unit status dots
        const unitItems = btn.querySelectorAll('.unit-status-item');
        unitItems.forEach((item, index) => {
            const unitData = plantData.units[index];
            if (!unitData) return;
            
            const statusDot = item.querySelector('.unit-status-dot');
            const statusText = item.querySelector('.unit-status-text');
            
            if (statusDot) {
                // Update status class
                statusDot.className = `unit-status-dot ${unitData.status}`;
            }
            ...
            if (statusText) {
                ...
                } else if (unitData.status === 'standby') {
                    statusText.textContent = 'Stby';
                } else {
        ...
        const summaryItems = btn.querySelectorAll('.summary-item');
        if (summaryItems[0]) summaryItems[0].innerHTML = `<span class="summary-dot running"></span> R: ${plantData.running_units || 0}`;
        if (summaryItems[1]) summaryItems[1].innerHTML = `<span class="summary-dot standby"></span> S: ${plantData.standby_units || 0}`;
        if (summaryItems[2]) summaryItems[2].innerHTML = `<span class="summary-dot offline"></span> O: ${plantData.offline_units || 0}`;
