...
function updatePlantStats(data) {
    ...
    //     elements.onlineUnits.textContent = data.onlineUnits;
    // }
    
    // Animate standby units
    if (elements.standbyUnits) {
        // const currentValue = parseInt(elements.standbyUnits.textContent) || 0;
        // animateValue(elements.standbyUnits, currentValue, data.standby_units, 1000, 0);
        elements.standbyUnits.textContent = data.standby_units;

    }
    
    // Animate running units
    if (elements.runningUnits) {
        // const currentValue = parseInt(elements.runningUnits.textContent) || 0;
        // animateValue(elements.runningUnits, currentValue, data.running_units, 1000, 0);
        elements.runningUnits.textContent = data.running_units;

        // Check if value changed (skip on first load)
        if (previousRunningUnits !== null && previousRunningUnits !== elements.runningUnits) {
            console.log(`🔄 Running units changed from ${previousRunningUnits} to ${elements.runningUnits}, reloading...`);
            reloadPage();
        }
        previousRunningUnits = elements.runningUnits;
    }
