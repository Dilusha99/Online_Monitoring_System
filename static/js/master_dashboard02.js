...
function updateMasterStats(data) {
    ...
    if (runningUnitsEl) {
        ...
        // Check if value changed (skip on first load)
        if (previousRunningUnits !== null && previousRunningUnits !== runningUnits) {
            console.log(`🔄 Running units changed from ${previousRunningUnits} to ${runningUnits}, reloading...`);
            reloadPage();
        }
        
        // Update previous value
        previousRunningUnits = runningUnits;
    }
    ...
    if (standbyUnitsEl) {
        ...
        // Check if value changed (skip on first load)
        if (previousStandbyUnits !== null && previousStandbyUnits !== standbyUnits) {
            console.log(`🔄 Standby units changed from ${previousStandbyUnits} to ${standbyUnits}, reloading...`);
            reloadPage();
        }
        
        // Update previous value
        previousStandbyUnits = standbyUnits;
    }
...
function updatePlantButtons(data) {
    ...
    buttons.forEach(btn => {
      ...
      // Update individual unit status dots
        ...
        if (plantData.units && Array.isArray(plantData.units)) {
            plantData.units.forEach((unit, index) => {
                const unitStatusDot = btn.querySelectorAll('.unit-status-dot')[index];
                const unitStatusText = btn.querySelectorAll('.unit-status-text')[index];
                ...
                if (unitStatusDot) {
                    ...
                    if (unitStatusText) {
                        unitStatusText.textContent = statusText;
                    }
                }
            });
        }
